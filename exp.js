...
async function experimentInit() {
  ...
  hello_text = new visual.TextStim({
    win: psychoJS.window,
    name: 'hello_text',
    text: 'Привет! \nСча будем проходить эксперимент!',
    font: 'Arial',
    units: undefined, 
  ...
  hello_key_resp = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
  ...
  bye_text = new visual.TextStim({
    win: psychoJS.window,
    name: 'bye_text',
    text: 'Подождите!\nДанные эксперимента сохраняются',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], draggable: false, height: 0.05,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
...
function helloRoutineBegin(snapshot) {
  return async function () {
    ...
    hello_key_resp.keys = undefined;
    hello_key_resp.rt = undefined;
    _hello_key_resp_allKeys = [];
    ...
    helloComponents.push(hello_key_resp);
...
function helloRoutineEachFrame() {
  return async function () {
    ...
    // *hello_key_resp* updates
    if (t >= 0.0 && hello_key_resp.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      hello_key_resp.tStart = t;  // (not accounting for frame time here)
      hello_key_resp.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { hello_key_resp.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { hello_key_resp.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { hello_key_resp.clearEvents(); });
    }
    
    // if hello_key_resp is active this frame...
    if (hello_key_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = hello_key_resp.getKeys({keyList: ['y','n','left','right','space'], waitRelease: false});
      _hello_key_resp_allKeys = _hello_key_resp_allKeys.concat(theseKeys);
      if (_hello_key_resp_allKeys.length > 0) {
        hello_key_resp.keys = _hello_key_resp_allKeys[_hello_key_resp_allKeys.length - 1].name;  // just the last key pressed
        hello_key_resp.rt = _hello_key_resp_allKeys[_hello_key_resp_allKeys.length - 1].rt;
        hello_key_resp.duration = _hello_key_resp_allKeys[_hello_key_resp_allKeys.length - 1].duration;
...
function helloRoutineEnd(snapshot) {
  return async function () {
    ...
    if (currentLoop instanceof MultiStairHandler) {
      currentLoop.addResponse(hello_key_resp.corr, level);
    }
    psychoJS.experiment.addData('hello_key_resp.keys', hello_key_resp.keys);
    if (typeof hello_key_resp.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('hello_key_resp.rt', hello_key_resp.rt);
        psychoJS.experiment.addData('hello_key_resp.duration', hello_key_resp.duration);
        routineTimer.reset();
        }
  ...
  return async function () {
    ...
    fetch('https://pipe.jspsych.org/api/data', {
        ...
        headers: {
         ...
         body: JSON.stringify({
            experimentID: 'KuUTz2KJvrjj', // * DATAPIPE EXP ID*
            filename: filename, 
            data: data,
         }),
